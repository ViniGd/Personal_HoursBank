...
def exportar_pdf():
    ...
    if arquivo:
        ...
        pdf.cell(200, 10, txt="Relatório de Horas", ln=True, align='C')
        pdf.ln(10)

        for registro in registros:
            pdf.cell(200, 10, txt=f"ID: {registro[0]}, Data: {registro[1]}, Horas: {decimal_para_horas_minutos(registro[2])}", ln=True)
...
def gerar_graficos():
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    cursor.execute('SELECT Data, Horas FROM registros')
    registros = cursor.fetchall()
    conn.close()

    datas = [datetime.strptime(registro[0], '%Y-%m-%d %H:%M:%S') for registro in registros]
    horas = [registro[1] for registro in registros]

    fig, ax = plt.subplots()
    ax.plot(datas, horas, marker='o')
    ax.set_xlabel('Data')
    ax.set_ylabel('Horas')
    ax.set_title('Horas por Dia')
    plt.xticks(rotation=45)
    plt.tight_layout()

    # Exibe o gráfico em uma nova janela
    janela_grafico = tk.Toplevel(root)
    janela_grafico.title("Gráfico de Horas")
...
def visualizar_dados():
    ...
    ttk.Button(frame_botoes, text="Exportar CSV", command=exportar_csv).pack(side=tk.LEFT, padx=5)
    ttk.Button(frame_botoes, text="Exportar PDF", command=exportar_pdf).pack(side=tk.LEFT, padx=5)
    ttk.Button(frame_botoes, text="Gerar Gráfico", command=gerar_graficos).pack(side=tk.LEFT, padx=5)
...
ttk.Button(frame_botoes, text="Importar Dados", command=importar_dados).pack(side=tk.LEFT, padx=5)
ttk.Button(frame_botoes, text="Modo Escuro", command=alternar_modo_escuro).pack(side=tk.LEFT, padx=5)
